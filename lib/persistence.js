...
class Persistence {
  ...
  async upsertGameRun(game) {
    ...
    if (game.game_metadata.phase === "waiting_room") {
      await this.pool.query("delete from turn_history where run_id = $1", [game.run_id]);
      await this.pool.query("delete from game_runs where run_id = $1", [game.run_id]);
      return;
    }
    ...
    const result = await this.pool.query(
      `
        select
          ...
          gr.updated_at
        from game_runs gr
        left join scenarios sc on sc.version = gr.scenario_version
        where gr.phase <> 'waiting_room'
        order by coalesce(gr.finished_at, gr.updated_at, gr.started_at) desc
      `,
    );
    return result.rows;
  }

  async loadGameRun(runId) {
    if (!this.enabled) {
      return null;
    }
    ...
    return {
      ...run,
      turn_history: turnsResult.rows.map((row) => row.payload),
    };
  }

  async deleteGameRun(runId) {
    if (!this.enabled) {
      return false;
    }

    await this.pool.query("delete from turn_history where run_id = $1", [runId]);
    const result = await this.pool.query("delete from game_runs where run_id = $1", [runId]);
